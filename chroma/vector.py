...
# Updated to use Gemini's latest embedding model
embeddings_model = GoogleGenerativeAIEmbeddings(model="gemini-embedding-2",api_key=config("GEMINI_API_KEY"))
...
def as_retriever():
    with tempfile.TemporaryDirectory() as tmpdir:
        vectorstore = Chroma.from_documents(
            ...
            # Updated to use Google's embedding model here too
            embedding=GoogleGenerativeAIEmbeddings(model="gemini-embedding-2", api_key=config("GEMINI_API_KEY")),
